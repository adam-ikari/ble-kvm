...
- **BLE HID KVM** — 通过蓝牙 BLE HID 连接最多 2 台电脑。第 3 台可通过 USB 设备模式连接。键盘和鼠标输入透明路由到当前选中的连接。默认广播名称为 `KVM-<MAC后4位>`（如 `KVM-A3F2`），可通过 Web 面板自定义。
- **USB 设备模式** — 通过 USB OTG 端口，以 USB HID 键盘/鼠标设备身份连接到主机。
- **USB 主机模式** — 接受外部 USB HID 键盘/鼠标，将其输入通过 BLE 转发给已连接的电脑。
- **配对模式** — 新 PC 连接需通过 Web 面板触发配对模式（60 秒超时）。已配对 PC 可自动重连，无需进入配对模式。
- **Web 认证持久化** — 认证令牌存储在 NVS 和浏览器 localStorage 中，关闭浏览器或重启设备后仍然有效。
...
- **双按钮操作** — 主按钮：短按切换电脑（PPT 模式：下一页），双击授权 Web 访问（PPT 模式：上一页），长按启动语音输入。副按钮：短按切换输入模式（PPT 模式：切回 KVM），长按 5 秒提示恢复出厂，长按 10 秒执行。
- **防空闲** — 定期向所有已连接电脑独立发送 HID 保活信号，防止主机休眠或锁屏。
- **Web 调试日志** — 通过 SSE 在 Web 仪表板上实时查看 ESP-IDF 日志输出。默认关闭，可在设置中开启。
- **恢复出厂设置** — 所有硬件统一两步确认：长按 5 秒警告（LED 快闪），继续按至 10 秒执行。5-10 秒之间松开取消。清除所有 NVS 设置并重启。
...
| 设置项 | 默认值 | 说明 |
|--------|-------|------|
| `device_name` | `KVM-XXXX` | BLE 广播名称（默认：MAC 地址派生） |
...
| `anti_idle_enabled` | false | 是否向所有已连接电脑发送 HID 保活信号 |
| `anti_idle_interval_sec` | 240 | 保活信号间隔（秒） |
...
| `POST` | `/api/pairing/start` | 进入 BLE 配对模式（60 秒超时） |
| `POST` | `/api/pairing/stop` | 退出 BLE 配对模式 |
...
**两步确认（所有硬件）：** 长按按钮 5 秒（LED 快闪警告），继续按住 5 秒（共 10 秒）确认执行。5-10 秒之间松开取消。清除所有 NVS 数据并重启设备。
